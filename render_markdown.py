...
def main():
    ...
    ap.add_argument("--base-json", required=False, help="Path to base size.json")
    ap.add_argument("--no-base", action="store_true", help="Generate report without base comparison")
    ...
    # Handle no base case
    if args.no_base or not args.base_json:
        # Generate single-column report showing only current state
        def used(d, k):
        ...
        def fmt_single_row(label: str, key: str, show_pct: bool) -> str:
            ...
            if show_pct:
                ...
                if pr_total > 0:
                    pr_pct = compute_pct(pr['ram'], pr_total, args.precision)
            
            pr_cell = fmt_with_pct(pr[key], pr_pct, args.precision) if show_pct or (key == 'ram' and pr_pct is not None) else fmt_bytes(pr[key])
            return f"| {label} | {pr_cell} |"

        rows = []
        rows.append(fmt_single_row('FLASH', 'flash', True))
        rows.append(fmt_single_row('DRAM', 'dram', True))
        rows.append(fmt_single_row('IRAM', 'iram', True))
        rows.append(fmt_single_row('RAM (DRAM+IRAM)', 'ram', False))

        md = [
            f"### ESP-IDF Size Report for '{args.app_name}'",
            "",
            f"| Metric | {args.head_name} |",
            "|---|---:|",
            *rows,
            "",
    ...
    # Original delta comparison logic
